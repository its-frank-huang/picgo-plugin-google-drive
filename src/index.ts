...
async function uploadProcess(ctx: IPicGo): Promise<void> {
  ...
  // Setup
  const browser = await chromium.launch(); // Set to true if you want to run headless
  ...
  for (const imgInfo of ctx.output) {
    ...
    try {
      ...
      ctx.log.info("created permissions");

      ctx.log.info("before get file");
      ...
      // Navigate to the Google Drive file page
      await page.goto(url);

      // Wait for the page to load and any necessary elements to be visible
      await page.waitForSelector('img[alt]:not([alt=""])');
      const sharedLink =
        (await page.locator('img[alt]:not([alt=""])').getAttribute("src")) ??
        url;
...
export = (ctx: IPicGo) => {
  ...
  async function testAfterAuthorizeFinish(
    ...
    return [
      {
        ...
        async handle(ctx: IPicGo, guiApi) {
          ...
          authorize(userConfig, ctx)
            .then(testAfterAuthorizeFinish)
            .catch(ctx.log.error);
          ctx.log.info(">> GDrive >> guiMenu > end");
        },
      },
      {
        label: ctx.i18n.translate<IGDriveLocalesKey>(
          "PIC_GDRIVE_MENU_TEST_UPLOAD"
        ),
        async handle(ctx: IPicGo, guiApi) {
      ...
      {
        label: ctx.i18n.translate<IGDriveLocalesKey>(
          "PIC_GDRIVE_MENU_CHECK_TOKEN_PATH"
        ),
        async handle(ctx: IPicGo, guiApi) {
          const tokenPath = getTokenPath(ctx);
          ctx.log.info(tokenPath);
          const title = ctx.i18n.translate<IGDriveLocalesKey>(
            "PIC_GDRIVE_MENU_CHECK_TOKEN_PATH"
          );
          const messageSuffix = ctx.i18n.translate<IGDriveLocalesKey>(
            "PIC_GDRIVE_MENU_CHECK_TOKEN_PATH_MESSAGE"
          );
